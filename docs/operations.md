...
`rivet-store-join-line` is centered on databases work in Swift. The technical target is: Develop a Swift command-oriented project for join scenarios with append-only fixtures, checkpoint recovery checks, and single-node deterministic mode.
...
The score is deterministic. The current threshold is 180. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 2.
